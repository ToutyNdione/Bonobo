...
class FriendController extends Controller
{
    ...
    public function indexAction()
    {
        ...
        $friends = $em->getRepository('BPBonoboBundle:Friend')->findAll();
     ...
     *
     * @Route("/addfriend", name="friend_new")
     * @Method({"GET", "POST"})
     */
    public function newAction(Request $request)
    {
        $friend = new Friend();
        $form = $this->createForm('BP\BonoboBundle\Form\FriendType', $friend);
        $form->handleRequest($request);

        if ($form->isSubmitted() && $form->isValid()) {
            $em = $this->getDoctrine()->getManager();
            $friendWithMe = $this->container->get('security.token_storage')->getToken()->getUser();
            $em->persist($friend);
    ...
    {
        $deleteForm = $this->createDeleteForm($friend);
        $editForm = $this->createForm('BP\BonoboBundle\Form\FriendType', $friend);
